        ...
        if mixup_fn is None:  # for mixup dont calculate accuracy
            ...
            if batch_idx % 100 == 10 and not test_contrastive_acc:
                print("Train Epoch: {:03d} Batch: {:05d}/{:05d} Loss: {:.4f} Acc@1: {:.2f}, Acc@5: {:.2f}"
                      .format(epoch, batch_idx, len(data_loader), loss.item(), 0 if contrastive else acc1.item(),
                              0 if contrastive else acc5.item()))  # , acc5.item()
...
def main(config, device, device_ids):
    ...
    if config.checkpoint_path:
        ...
        if not config.test_contrastive_acc and not config.eval and config.num_classes != state_dict[
            'classifier.weight'].size(0):  # not
    ...
    for epoch in range(1, config.epochs + 1):
        ...
        if not config.eval:
            ...
            result = train_epoch(epoch, model, train_dataloader, criterion, optimizer, lr_scheduler, train_metrics,
                                 device,
                                 contrastive=config.contrastive, test_contrastive_acc=config.test_contrastive_acc,
