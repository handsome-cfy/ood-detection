...
def get_transform(contrastive=True, train=True, image_size=224, dataset='cifar10', network='vit', albumentations=False,
                  no_train_aug=False, deit=False):
    scale = (0.2, 1.)
    if network == 'vit':
        if deit:
...
class cifar10DataLoader(DataLoader):
    def __init__(self, data_dir, split='train', image_size=224, batch_size=16, num_workers=8, contrastive=False,
                 albumentation=True, net='vit', no_train_aug=False, in_dataset=None, deit=False):
        if split == 'train':
            train = True
...
class stl10DataLoader(DataLoader):
    def __init__(self, data_dir, split='train', image_size=96, batch_size=16, num_workers=8, contrastive=False,
                 albumentation=True, net='vit', no_train_aug=False, in_dataset=None, deit=False):
        if split == 'train':
            train = True
            transform = get_transform(contrastive, train=train, image_size=image_size, dataset=in_dataset,
        ...
        self.dataset = STL10(root=data_dir, split=split if split == 'train' else 'test', transform=transform,
                             download=True)
...
class CUBDataLoader(DataLoader):
    def __init__(self, data_dir, split='val', image_size=224, batch_size=16, num_workers=8, contrastive=False,
                 albumentation=True, net='vit', no_train_aug=False, in_dataset=None, deit=False):
        train = False
        transform = get_transform(train=train, image_size=image_size, dataset=in_dataset, network=net, deit=deit)
...
class Caltech256DataLoader(DataLoader):
    def __init__(self, data_dir, split='val', image_size=224, batch_size=16, num_workers=8, contrastive=False,
        ...
        self.dataset = ImageFolder(root=os.path.join(data_dir, 'caltech256'),
                                   transform=transform)  # Caltech256(root=data_dir, transform=transform, download=True)
...
class DogsDataLoader(DataLoader):
    def __init__(self, data_dir, split='val', image_size=224, batch_size=16, num_workers=8, contrastive=False,
        ...
        self.dataset = ImageFolder(root=os.path.join(data_dir, 'Dogs'),
                                   transform=transform)  # Caltech256(root=data_dir, transform=transform, download=True)
...
class DtdDataLoader(DataLoader):
    def __init__(self, data_dir, split='val', image_size=224, batch_size=16, num_workers=8, contrastive=False,
        ...
        self.dataset = ImageFolder(root=os.path.join(data_dir, 'Dtd'),
                                   transform=transform)  # Caltech256(root=data_dir, transform=transform, download=True)
...
class Places365DataLoader(DataLoader):
    def __init__(self, data_dir, split='val', image_size=224, batch_size=16, num_workers=8, contrastive=False,
        ...
        self.dataset = ImageFolder(root=os.path.join(data_dir, 'Places365'),
                                   transform=transform)  # Caltech256(root=data_dir, transform=transform, download=True)
...
class FoodDataLoader(DataLoader):
    def __init__(self, data_dir, split='val', image_size=224, batch_size=16, num_workers=8, contrastive=False,
        ...
        self.dataset = ImageFolder(root=os.path.join(data_dir, 'Food'),
                                   transform=transform)  # Caltech256(root=data_dir, transform=transform, download=True)
...
class Imagenet_resizeDataLoader(DataLoader):
    def __init__(self, data_dir, split='val', image_size=224, batch_size=16, num_workers=8, contrastive=False,
        ...
        self.dataset = ImageFolder(root=os.path.join(data_dir, 'Imagenet_resize'),
                                   transform=transform)  # Caltech256(root=data_dir, transform=transform, download=True)
...
def create_dataloaders(config, no_train_aug=False, out_dataset=False):
    ...
    # create dataloader
    print("Creating dataloaders for {0} with network {1} and ablumentations {2}".format(dataset, config.model,
                                                                                        config.albumentation))
    train_dataloader = eval("{}DataLoader".format(dataset))(
        data_dir=config.out_data_dir if out_dataset else config.data_dir,
